...
class AnomalyCLIPModule(LightningModule):
    ...
    def on_test_epoch_start(self):
        # initialize containers for accumulating per-batch outputs
        self._test_abnormal_scores = []
        self._test_labels = []
        self._test_class_probs = []
    ...
    def test_step(self, batch: Any, batch_idx: int):
        ...
        self._test_abnormal_scores.append(abnormal_scores)
        self._test_labels.append(labels)
        self._test_class_probs.append(class_probs)

    @rank_zero_only
    def on_test_epoch_end(self):
        abnormal_scores = torch.cat(self._test_abnormal_scores)
        labels = torch.cat(self._test_labels)
        class_probs = torch.cat(self._test_class_probs)
